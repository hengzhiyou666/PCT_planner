...
bool GPMPOptimizer::GenerateTrajectory(const std::vector<PathPoint>& input_path,
  ...
  for (int i = 0; i < N; ++i) {
    ...
    std::cout << "Inserted value " << i << std::endl;
  ...
  std::cout << "Adding PriorFactor6..." << std::endl;
  graph.add(PriorFactor6(gtsam::Symbol('x', 0), x0, sigma_initial));
  std::cout << "Added PriorFactor6." << std::endl;
  factor_idx++;
  for (int i = 1; i < N; ++i) {
    if (debug_ || i == 1) std::cout << "Processing factor loop " << i << std::endl;
    gtsam::Key last_x = gtsam::Symbol('x', i - 1);
    gtsam::Key this_x = gtsam::Symbol('x', i);
    graph.add(GPPriorFactor(last_x, this_x, dt, kQc));
    if (debug_ || i == 1) std::cout << "Added GPPriorFactor " << i << std::endl;
    factor_idx++;
    for (int j = 0; j < interpolate_num_; ++j) {
      double height_hint =
  ...
  std::cout << "Factor loop finished." << std::endl;

  std::cout << "Adding goal prior..." << std::endl;
  graph.add(PriorFactor6(gtsam::Symbol('x', N - 1), xN, sigma_goal));
  std::cout << "Added goal prior." << std::endl;
  factor_idx++;
